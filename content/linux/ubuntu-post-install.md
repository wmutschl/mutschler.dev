...
summary: In the following I will go through my post installation steps on Ubuntu Desktop, i.e. apps, system configuration, remote access setup, security hardening, and various optimizations.
header:
  image: "Ubuntu_25.10.png"
  caption: "Image credit: [**By Canonical Limited - Free software screenshot, GPL, https://commons.wikimedia.org/w/index.php?curid=176484298**](https://en.wikipedia.org/wiki/Ubuntu#/media/File:Ubuntu_25.10_default_desktop_-_English.png)"
...
In the following I will go through my post installation steps on Ubuntu Desktop.
I use this machine primarily as a server, but still need a desktop environment for certain tasks. The guide therefore covers apps, system configuration, remote access setup, security hardening, and various optimizations that I perform after installing Ubuntu Desktop 25.10 with BTRFS and LUKS encryption.
...
sudo snap refresh
```
Sometimes (e.g. if new hardware is activated), there is an interruption to the update process.
In this case, one needs to fix broken package installations with `sudo dpkg --configure -a` and then continue with the update process.
...
This [video](https://www.youtube.com/watch?v=wJN3e8Usmzw) explains the problem and solution.
To fix it, we need to add some kernel parameters to GRUB:

```fish
sudo nano /etc/default/grub
```

Find the line that starts with `GRUB_CMDLINE_LINUX_DEFAULT` and add `amd_iommu=on iommu=pt` to it. Also change `GRUB_TIMEOUT` to 3 seconds and `GRUB_RECORDFAIL_TIMEOUT` to 5 seconds. For example:
...
{{< callout note >}}
I use the same LUKS passphrase across all disks (including the system disk), so at reboot I only enter it once and it's automatically passed to the other disks. Good to know :-)
{{< /callout >}}
...
# NAME             MAJ:MIN RM   SIZE RO TYPE  MOUNTPOINTS
# sda                8:0    0   3.6T  0 disk
# └─crypt_backup1  252:1    0   3.6T  0 crypt /btrfs_backup
# sdb                8:16   0   3.6T  0 disk
# └─crypt_backup2  252:0    0   3.6T  0 crypt
# sdc                8:32   0 119.2G  0 disk
# ├─sdc1             8:33   0     1G  0 part  /boot/efi
# └─sdc2             8:34   0 118.1G  0 part  /
# nvme0n1          259:0    0   1.7T  0 disk
...
# ├─nvme0n1p3      259:3    0   9.8G  0 part
# └─nvme0n1p4      259:4    0   1.6T  0 part
#   └─crypt_docker 252:2    0   1.6T  0 crypt /home/wmutschl/docker
#                                             /btrfs_docker
#                                             /home/wmutschl/vm
...
# mount: /btrfs_backup: /dev/mapper/crypt_backup2 already # mounted or mount point busy.
#        dmesg(1) may have more information after failed mount system call.

sudo btrfs filesystem show /btrfs_backup
# Label: none  uuid: e1963877-fc4d-4412-ba2e-f67e2b865f4b
# 	Total devices 2 FS bytes used 2.14TiB
# 	devid    1 size 3.64TiB used 2.21TiB path /dev/mapper/crypt_backup1
# 	devid    2 size 3.64TiB used 2.21TiB path /dev/mapper/crypt_backup2
...
# 	devid    1 size 1.64TiB used 1.21TiB path /dev/mapper/crypt_docker
```
...
```fish
sudo nano /etc/fstab
# /dev/disk/by-uuid/48c5cb40-0a06-4222-944c-4b9ccca36fc2 /           btrfs defaults,compress=zstd:1,subvol=@      0 0
# /dev/disk/by-uuid/48c5cb40-0a06-4222-944c-4b9ccca36fc2 /home       btrfs defaults,compress=zstd:1,subvol=@home  0 0
# /dev/disk/by-uuid/48c5cb40-0a06-4222-944c-4b9ccca36fc2 /btrfs_pool btrfs defaults,compress=zstd:1,subvolid=5    0 0
```
...
### Remote desktop
For now I use Gnome's remote desktop feature which runs over RDP.
Unfortunately, it does not work for me headless.
As I have a [JetKVM](https://jetkvm.com) attached to my system, I am fine for now, but let me know of a solution that works headless with Wayland.
...
- Bluetooth: Nothing to do
- Displays: Check settings
- Sound: I mute all devices, no *Startup Sound*
- Power:
  - Set *Power Mode* to *Balanced*
  - *Power Button Behavior* to *Power Off*
  - Turn on *Automatic Screen Blank* and set *Delay* to 5 minutes
  - Deactivate *Automatic Suspend*
- Multitasking:
  - Deactivate *Hot Corner*
  - Activate *Active Screen Edges*
  - Use *Dynamic Workspaces*
  - Multi Monitor: Select *Workspaces on primary display only*
  - App Switching: Select *Include apps from all workspaces* and *Include apps from all monitors*
- Appearance:
  - Style: *Default*
  - Accent Color: *Teal*
  - Pick a Background
- Ubuntu Desktop:
  - Desktop Icons: *Size=Normal*, *Position of New Icons=Bottom Right*, *Show Home Folder=off*
  - Dock
    - activate *Auto-hide the Dock*
    - activate *Panel Mode*
    - *Icon Size=48*
    - *Show on=Primary Display(1)*
    - *Position on Screen=Bottom*
    - Configure Dock Behavior: Deactivate *Show Volumes and Devices*, activate *Show Trash*
    - Activate *Enhanced Tiling*, *Tiling Popup*, and *Tile Groups*
- Apps: go thrpough all apps and decide whether you need them or uninstall these.
- Notifications: go through all options and deactivate all that are not needed.
- Search: Activate everything
- Online Accounts: I don't have any use for these
- Sharing: *Device Name* should be *simba* and *Media Sharing* should be *Off*
- Wellbeing: Nothing to do
- Mouse & Touchpad: Nothing to do
- Keyboard: Nothing to do
- Color management: Nothing to do
- Printers: Nothing to do
- Accessibility: Nothing to do
- Privacy & Security
  - Screen Lock
    - Blank Screen Delay: 5 minutes
    - Automatic Screen Lock: activate
    - Automatic Screen Lock Delay: Screen Turns Off
    - Lock Screen Notifications: activate
    - Lock Screen Suspend: deactivate
  - Location: Deactivate *Automatic Device Location*
  - File History: Deactivate
  - Diagnostics: Nothing to do
  - Connectivity: Deactivate *Connectivity Checking*
  - Devices: Nothing to do
- System
  - Region & Language: Click on *Manage Installed Languages* and remove unnecessary languages. Then click on *Languages* and update these. Also check the settings under *Your Account*.
  - Date & Time: Nothing to do
  - Users: Update the icon.
  - Remote Desktop:
    - Desktop Sharing: Turn on *Desktop Sharing* and *Remote Control*, change the *Username* and *Password*
    - Remote Login: Unforuntately, this does not work for me.
  - Secure Shell: Nothing to do
  - About: Nothing to do
